...
## v3 enhancements (this release)
- Auto-fetcher: `scripts/fetch_universe.py` fetches NSE/Nifty Total Market constituents (pluggable sources).
- Position sizing & sector caps: daily output includes volatility-aware weights, portfolio heat cap, and max positions per sector.
- Backtest charts: `scripts/backtest_viz.py` generates equity curve, monthly returns, drawdown, and trade distribution charts as PNGs.
- Daily output now includes: `screened_stocks.csv`, `sector_leadership.csv`, `setup_mix.csv`, plus `position_size_pct` and `position_type` columns.

## Quick start
1. Run `python -m scripts.fetch_universe` to populate `data/universe.csv` with NSE constituents.
2. Trigger the weekday scan via `workflow_dispatch` to validate.
3. Run `python -m scripts.backtest` then `python -m scripts.backtest_viz` for the 15-year report with charts.
