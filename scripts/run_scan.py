...
def main():
    ...
    if out.empty:
        ...
        # Position sizing: volatility-aware, capped by risk limits
        max_w = cfg["risk"]["max_single_position_weight"]
        port_heat = cfg["risk"]["portfolio_heat"]
        max_sector = cfg["risk"]["max_sector_positions"]
        out["vol_norm"] = 1.0 / (out["atr_pct"] + 0.01)
        out["vol_norm"] = out["vol_norm"] / (out["vol_norm"].sum() + 1e-9)
        out["raw_weight"] = out["vol_norm"] * port_heat
        out["raw_weight"] = out["raw_weight"].clip(upper=max_w)
        out["adj_weight"] = out["raw_weight"] / (out["raw_weight"].sum() + 1e-9) * port_heat
        out["position_size_pct"] = (out["adj_weight"] * 100).round(2)

        # Sector caps: enforce max positions per sector
        sector_counts = out.groupby("sector", dropna=False).cumcount()
        sector_mask = sector_counts < max_sector
        out = out[sector_mask]

        out["rank"] = range(1, len(out) + 1)
        out["setup_type"] = out["z20"].apply(lambda z: "Trend Pullback" if z < -0.5 else "Trend Continuation")
        out["position_type"] = out["composite_score"].apply(lambda s: "Core" if s >= out["composite_score"].quantile(0.75) else "Satellite")
